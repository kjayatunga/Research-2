...
\JL{KASUN: I don't understand the above paragraph. What is the main point of this paragraph? Are you trying to show that the only the terms dependent on $\dot{y}$ are dissipative, and so only those terms contribute to the transfer of energy or power to the body? That is true, but that is also true for VIV, so I don't see how that shows that the ``system is governed by the transverse velocity rather than the natural frequency''. I definitely don't understand that last sentence. Do you mean ``power'' when you say ``system''? The problem is that, as I've pointed out, the exact same argument regarding dissipation applies to VIV, which is strongly dependent on the natural frequency.}
...
where the coefficients can be regrouped into combinations of non-dimensional groups, and rewritten as
\begin{equation}
...
\JL{KASUN: why have you established that it is the damping, not $U^*$, that is important, but here you persist with presenting results in terms of $U^*$? How do these regions correspond to regions on the curve in terms of your damping parameter (that I have labelled $\Gamma_2$)? Can you define these three regions in terms of this new parameter?}
...
\JL{KASUN: I don't understand the point of the above paragraph. What are you trying to say? That maximum power output occurs when the tranverse velocity is such that the lift force is a maximum?}
...
\JL{KASUN: nothing to change here, just wanted to note that this paragraph is good. Clear, and to the point.}
...
\JL{KASUN, again this last paragraph is all in terms of $U^*$, but you establish earlier that $U*^$ is not the important parameter, your new damping parameter (which is actually a mass-damping parameter) is the critical parameter. Can this argument be rewritten in terms of this new parameter?}
